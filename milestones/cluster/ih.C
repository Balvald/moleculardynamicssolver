...
//
// Modified by: Florian Probst 2024/07/19
//
...
int main( int argc, char *argv[] )
{
  ...
  // Skipping any printing to terminal.
  /*for( int i=0; i<num; i++ )
  {
    cout << p[i].s << " " << p[i].v*d << endl;
    cout << p[i].s << " " << p[i].v.x*d << " " << p[i].v.y*d << " " <<  p[i].v.z*d << endl;
  }*/

// using d = 2.9 gives us a mackay icosahedron that looks pretty much exactly as the ones that were given in the course.

  Positions_t pos(3, num);
  Names_t names(num);
  for (int i = 0; i < num; i++)
  {
    names(i) = "Au";
    pos(0, i) = 5 + 2.34615*n + (p[i].v.x) * d;
    pos(1, i) = 5 + 2.34615*n + (p[i].v.y) * d;
    pos(2, i) = 5 + 2.34615*n + (p[i].v.z) * d;
    //pos(0, i) = (p[i].v.x) * d;
    //pos(1, i) = (p[i].v.y) * d;
    //pos(2, i) = (p[i].v.z) * d;
  }

  Atoms atoms = Atoms(names, pos);

  std::string filename = "mackay_cluster_" + std::to_string(num) + ".xyz";

  write_xyz_filename(filename, atoms);
