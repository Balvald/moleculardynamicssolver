        ...
        if (rank == 0)
        {
            if (i % relaxation_time == 0)
            {
                ...
                if (q_delta > 0.0 && i > eq_steps)
                {
                    ...
                    atoms.velocities *= sqrt(1.0 + (q_delta/kinetic_energy)); // sqrt(1.0 + std::abs((q_delta+kinetic_abs)/(kinetic_abs-1.0)));
                }
            }

            if (((int)i % (relaxation_time)) > (relaxation_time/2)  || (int) i == 0)
            {
                avg_temp += 2.0 * kinetic_energy / (3.0 * nb_global);
            }
            ...
            if (((int) i+1) % (relaxation_time) == 0 || (int) i == 0)
            {
                ...
                outdata << "[" << i << "," << potential_energy << "," << kinetic_energy << "," << potential_energy + kinetic_energy << "," << (2.0 * kinetic_energy / (3.0 * nb_global)) << "," << avg_temp  << "]" << std::endl;
